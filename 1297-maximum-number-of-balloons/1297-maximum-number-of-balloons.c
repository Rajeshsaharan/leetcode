int check_ballon(int * arr, int * index){
    int i = 0;
    int a_value = arr[index[i]]; printf("a_value %d\n", a_value);
    ...
    if(a_value >=1 && b_value >=1 && l_value >=2 && n_value >=1 && o_value >=2){
        arr[index[i]] = a_value -1 ; printf(" must be zero -> %d", arr[index[0]]);
        arr[index[i+1]] = b_value -1 ;
        arr[index[i+2]] = l_value -2;
        arr[index[i+3]] = n_value -1;
